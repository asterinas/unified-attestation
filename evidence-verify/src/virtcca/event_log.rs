use crate::eventlog_rs::{self, Eventlog};
...
fn file_reader(file_path: &str) -> io::Result<HashSet<String>> {
...
impl EventVerify {
        ...
        for event_entry in event_log.log.iter() {
            if event_entry.event_type == grub_event_type {
                let Some(digest) = event_entry
                    ...
                    log::warn!("Skipping EVENT GRUB image event without digest");
                    continue;
                };
                ...
                for (&key, &pattern) in event_descs.iter() {
                    ...
                    if event_desc.contains(pattern) {
                        let Some(digest) = event_entry
                            ...
                            log::warn!("Skipping EVENT entry '{key}' without digest");
                            continue;
                        };
                        state
                            .state_hash
    ...
    fn validate_event_log_digests(event_log: &Eventlog) -> Result<()> {
        for (index, event_entry) in event_log.log.iter().enumerate() {
            match event_entry.digests.first() {
                ...
                Some(_) => bail!("EVENT log entry {index} has an empty digest"),
                None => bail!("EVENT log entry {index} has no digest"),
            }
        }
    ...
    pub fn event_verify(
        ...
        let event_parsed = eventlog_rs::Eventlog::try_from(event_log)
            .map_err(|err| anyhow!("failed to parse EVENT log: {err}"))?;
        EventVerify::validate_event_log_digests(&event_parsed)?;
        ...
        let event_refs = file_reader(EVENT_REFERENCE_FILE).with_context(|| {
            format!("failed to read VirtCCA event reference {EVENT_REFERENCE_FILE}")
        })?;
