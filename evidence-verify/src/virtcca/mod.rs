...
impl Evidence {
    ...
    pub fn verify(user_data: &[u8], evidence: &[u8]) -> Result<TeeClaim> {
        ...
        // verify ima
        let ima: serde_json::Value = match virtcca_ev.ima_log {
            Some(ima_log) => {
                log::info!("get ima log");
                #[cfg(feature = "ima-verifier")]
                {
                    let app_id = hex::encode(&evidence.cvm_token.rim);
                    VirtCCAImaVerify::default().ima_verify(
                        &ima_log,
                ...
                bail!("VirtCCA evidence contains ima_log, but ima-verifier feature is disabled");
            }
            None => {
                log::info!("no ima log");
                json!({})
        ...
        evidence.parse_claim_from_evidence(ima, event_log)
    }
    pub fn parse_evidence(evidence: &[u8]) -> Result<TeeClaim> {
        ...
        let event_log = json!("");
        // parsed TeeClaim
        let claim = evidence.parse_claim_from_evidence(ima, event_log)?;
    ...
    fn verify_dev_cert_chain(dev_cert: &X509) -> Result<()> {
        ...
        let sub_cert_file = std::fs::read(sub_cert_path)
            .with_context(|| format!("failed to read VirtCCA sub cert {sub_cert_path}"))?;
        let sub_cert = X509::from_pem(&sub_cert_file)
            .with_context(|| format!("failed to parse VirtCCA sub cert {sub_cert_path}"))?;
        let root_cert_file = std::fs::read(root_cert_path)
            .with_context(|| format!("failed to read VirtCCA root cert {root_cert_path}"))?;
        let root_cert = X509::from_pem(&root_cert_file)
            .with_context(|| format!("failed to parse VirtCCA root cert {root_cert_path}"))?;
    ...
    fn compare_with_ref(&mut self) -> Result<()> {
        let ref_file = std::fs::read(VIRTCCA_REF_VALUE_FILE).with_context(|| {
            format!("failed to read VirtCCA reference value {VIRTCCA_REF_VALUE_FILE}")
        })?;
        let js_ref = serde_json::from_slice(&ref_file).with_context(|| {
            format!("failed to parse VirtCCA reference value {VIRTCCA_REF_VALUE_FILE}")
        })?;
        match js_ref {
